...
local function createForecast()
	...
	local cTime = exports.frp_daytime_control:getCurrentTime()

	for i = 0, #weatherForecast do
		local d, h, m, s, weather, wind

		if i == 0 then
			d, h, m, s = TimeToDHMS(cTime)
			weather = currentWeather
			wind = currentWindDirection
		else
			local time = (timeIsFrozen and cTime or (cTime + weatherInterval * i) % weekLength)
...
local function syncTime(player, tick)
	...
	local timeTransition = ((dayLength - (cTime % dayLength) + tick) % dayLength <= tick and 0 or syncDelay)
	local day, hour, minute, second = TimeToDHMS(cTime)
...
AddEventHandler("weathersync:requestUpdatedAdminUi", function()
	local cTime = exports.frp_daytime_control:getCurrentTime()
	TriggerClientEvent("weathersync:updateAdminUi", source, currentWeather, cTime, currentTimescale, currentWindDirection, currentWindSpeed, syncDelay)
end)
...
RegisterCommand("weather", function(source, args, raw)
	if API.IsPlayerAceAllowedGroup( source, "admin") then
